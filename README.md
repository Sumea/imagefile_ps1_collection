...
#### General usage:   
These scripts are meant for command line usage. The needed encoder/decoder binaries and these .ps1 scripts need to be in %PATH%. The scripts have my subjective "Generally high quality" defaults, so running the script alone from terminal targeting a directory with images will end up with both original and new images next to each other in a directory. These scripts may be very compatible with OneCommander's implementation of automation scripts, this is untested.   
...
But what will it do? This is another personal script meant for image file sub folder  jungle situations that one might want to compress into the main folder. It will target the directory the script is run from as the main directory, move all files to the main directory with the sub folders added to the file's name with separators.    
Example: mainfolder/images/2016/08/photo004.jpg -> mainfolder/images_2016_08_photo004.jpg   
No pruning logic either, just use your explorer and del key like in the olden days.   
